...
local function attemptTeleport(serverId)
    ...
    if not success then
        ...
        task.wait(5)  -- Wait for 5 seconds before retrying
        attemptTeleport(serverId)  -- Retry teleportation
    else
        print("Teleport successful to server: " .. serverId)
    end
end
    ...
    if success and result and result.data then
        for _, server in ipairs(result.data) do
            ...
            if server.id ~= game.JobId and server.playing < server.maxPlayers and playerCountAfterJoin <= 7 and not visitedServers[server.id] then
                ...
                attemptTeleport(server.id)  -- Use our retry logic here
                return
            end
        end
